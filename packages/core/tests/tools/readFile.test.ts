...
describe("readFile tool", () => {
  ...
  describe("line range support", () => {
    ...
    it("clamps lineEnd > total lines silently", async () => {
      ...
      expect(result.content).toContain("--- Lines 1-5 of 5 ---");
    });
    ...
    it("returns error when lineStart > total lines", async () => {
      ...
      expect(result.content).toContain("lineStart out of range");
    });
    ...
    it("returns error when lineStart > lineEnd", async () => {
      ...
      expect(result.content).toContain("lineStart must be <= lineEnd");
    });
  });
  ...
  describe("size limit", () => {
    it("returns error for file >64KB without range", async () => {
      ...
      expect(result.content).toContain("Extracted text exceeds 64 KB");
    });
    ...
    it("succeeds for file >64KB with range under 64KB", async () => {
      ...
      expect(result.content).not.toContain("Extracted text exceeds 64 KB");
      expect(result.content).toContain("--- Lines 1-10 of");
    });
  });
  ...
  describe("binary detection", () => {
    it("returns error for binary file with null byte", async () => {
      ...
      expect(result.content).toContain("Unsupported binary format");
    });
  ...
  describe("PDF support", () => {
    it("reads PDF file and extracts text with page count header", async () => {
      ...
      expect(result.content).toContain("--- PDF, 1 pages ---");
